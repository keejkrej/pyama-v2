...
export function createSelection(scan: WorkspaceScan, initial?: Partial<Selection>): Selection {
  return {
...
export function coerceSelection(scan: WorkspaceScan, selection: Selection): Selection {
  return {
...
export function makeSourceKey(source: Source): string {
  return `${source.kind}:${source.path}`;
}

export function makeFrameKey(source: Source, selection: Selection): string {
