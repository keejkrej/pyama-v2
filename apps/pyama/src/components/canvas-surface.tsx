...
import type { FrameResult, CanvasStatusTone } from "@/lib/contracts";
import {
...
function messageToneClasses(tone: CanvasStatusTone | undefined) {
  if (tone === "error") {
    return {
...
export default function CanvasSurface({
  ...
  const getFramePointFromClient = useCallback((clientX: number, clientY: number): CanvasFramePoint | null => {
    const cached = latestFrameRef.current;
    const view = viewportRef.current;
  ...
  const toVirtualPointerEvent = useCallback(
    (event: ReactPointerEvent<HTMLCanvasElement>): CanvasPointerEvent => ({
  ...
  const toVirtualWheelEvent = useCallback(
    (event: ReactWheelEvent<HTMLCanvasElement>): CanvasWheelEvent => ({
