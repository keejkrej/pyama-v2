...
import type { FrameResult, ViewerCanvasStatusTone } from "@/shared/contracts";
import {
  clamp,
...
export default function AlignCanvasSurface({
  frame,
  grid,
  previewGrid,
  excludedCells,
  ...
  const getFramePointFromClient = useCallback((clientX: number, clientY: number): AlignCanvasFramePoint | null => {
    const cached = latestFrameRef.current;
    const view = viewportRef.current;
